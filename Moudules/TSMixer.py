...
class TS_Mixer(nn.Module):
    ...
    def __init__(self, vocab_size: int, num_mixers: int, max_seq_len: int, hidden_dim: int, mlp_hidden_dim: int,
                 num_classes, **kwargs):
...
class MixerLayer(nn.Module):
    ...
    def forward(self, x):
        B, N, D = x.shape
        x1 = self.FC1(x)
        x_shift1 = torch.zeros(B, N,D, device=x.device)
        x_shift2 = torch.zeros(B, N, D, device=x.device)
